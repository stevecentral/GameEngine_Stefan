...
{
    //scenario dealing with collision from the right side by acccelerating rightward
    ...
    //scenario dealing with collision from the bottom side by acccelerating downward
    ...
    //scenario dealing with collision from the left side by acccelerating leftward
    ...
    //scenario dealing with collision from the top side by acccelerating upward
...
void PhysicsSystem::checkCollisionSides(ECS::ComponentHandle<Transform> transform, ECS::ComponentHandle<BoxCollider> touchingBox, sf::RectangleShape touchedRectangle)
{
    ...
    if (transform->ySpeed > 0 &&
        touchedRectBottom + transform->xSpeed > touchedRectTop &&
